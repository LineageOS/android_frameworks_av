...
class LegacyGraphicsTracker;

struct LegacyGraphicBufferAllocator : public BnGraphicBufferAllocator {
  public:
    // HAL interfaces
    ...
    ::ndk::ScopedAStatus getWaitableFd(::ndk::ScopedFileDescriptor* _aidl_return) override;
     ...
     */
    c2_status_t allocate(uint32_t width, uint32_t height, ::android::PixelFormat format,
     ...
     */
    static std::shared_ptr<LegacyGraphicBufferAllocator> CreateLegacyGraphicBufferAllocator(
            int maxDequeueCount);

  private:
    LegacyGraphicBufferAllocator(int maxDequeueCount);

    std::shared_ptr<LegacyGraphicsTracker> mGraphicsTracker;

...
class LegacyGraphicsTracker {
  public:
    static std::shared_ptr<LegacyGraphicsTracker> CreateLegacyGraphicsTracker(int maxDequeueCount) {
        LegacyGraphicsTracker* p = new LegacyGraphicsTracker(maxDequeueCount);
        std::shared_ptr<LegacyGraphicsTracker> sp(p);
        return sp;
    }

    ~LegacyGraphicsTracker();
  ...
  private:
    ...
    struct BufferItem {
        ...
        BufferItem(uint32_t generation, int slot, const sp<GraphicBuffer>& buf,
                   const sp<Fence>& fence);

        // Create from an AHB (no slot information)
        // Should be attached to IGBP for rendering
        BufferItem(uint32_t generation, AHardwareBuffer* pBuf, uint64_t usage);
    ...
    struct BufferCache {
        ...
        BufferCache()
            : mBqId{0ULL}, mUsage{0ULL}, mGeneration{0}, mIgbp{nullptr}, mNumAttached{0} {}
  ...
  private:
    explicit LegacyGraphicsTracker(int maxDequeueCount);
    ...
    void clearCacheIfNecessaryLocked(const std::shared_ptr<BufferCache>& cache,
    ...
    c2_status_t requestRender(uint64_t bid, std::shared_ptr<BufferCache>* cache,
                              std::shared_ptr<BufferItem>* pBuffer, bool* fromCache,
                              bool* updateDequeue);
    c2_status_t requestAttachForRender(const C2ConstGraphicBlock& blk, const sp<Fence>& fence,
    ...
    void commitAllocate(c2_status_t res, const std::shared_ptr<BufferCache>& cache, bool cached,
                        int slotId, const sp<Fence>& fence, std::shared_ptr<BufferItem>* buffer,
                        bool* updateDequeue);
    ...
    void commitRender(const std::shared_ptr<BufferCache>& cache,
                      ...
                      const std::shared_ptr<BufferItem>& oldBuffer, bool bufferReplaced,
                      bool* updateDequeue);
    ...
    c2_status_t _allocate(const std::shared_ptr<BufferCache>& cache, uint32_t width,
                          ...
                          int* rSlotId, sp<Fence>* rFence, std::shared_ptr<BufferItem>* buffer);

    c2_status_t _allocateDirect(uint32_t width, uint32_t height, PixelFormat format, uint64_t usage,
                                AHardwareBuffer** buf, sp<Fence>* fence);

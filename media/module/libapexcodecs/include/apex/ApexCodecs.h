 ...
 */
typedef void *_Nullable(*ApexCodec_MapFn)(
 ...
 */
typedef int (*ApexCodec_UnmapFn)(void *_Nullable addr, size_t length);
 ...
 *
 * \param store the component store
 * \param componentName the name of the component in ASCII encoding
 * \return a function pointer for mapping memory, or nullptr if not available.
 */
ApexCodec_MapFn _Nullable ApexCodec_GetMapFn(
        ApexCodec_ComponentStore *_Nonnull store,
 ...
 */
ApexCodec_UnmapFn _Nullable ApexCodec_GetUnmapFn(
        ApexCodec_ComponentStore *_Nonnull store,
        const char *_Nonnull componentName) __INTRODUCED_IN(37);

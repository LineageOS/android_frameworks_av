...
#define LOG_TAG "DrcPresModeWrapRustAac"
// #define LOG_NDEBUG 0
#include <utils/Log.h>
...
DrcPresModeWrapRustAac::DrcPresModeWrapRustAac()
{
    mDataUpdate = true;
...
void
DrcPresModeWrapRustAac::setDecoderHandle(const HANDLE_AACDECODER handle)
{
    mHandleDecoder = handle;
}

void
DrcPresModeWrapRustAac::submitStreamData(
        StreamInfo* pStreamInfo,
        OutputInfo* pOutputInfo,
        MetadataInfo* pMetadataInfo)
{
    assert(pStreamInfo);
    assert(pOutputInfo);
    assert(pMetadataInfo);

    if (mStreamPRL != pMetadataInfo->drc_program_reference_level) {
        mStreamPRL = pMetadataInfo->drc_program_reference_level;
        mDataUpdate = true;
    ...
    if (mStreamDRCPresMode != pMetadataInfo->drc_presentation_mode) {
        mStreamDRCPresMode = pMetadataInfo->drc_presentation_mode;
        mDataUpdate = true;
#ifdef DRC_PRES_MODE_WRAP_DEBUG
    ...
    if (mStreamNrOutChan != pOutputInfo->num_channels) {
        mStreamNrOutChan = pOutputInfo->num_channels;
        mDataUpdate = true;
#ifdef DRC_PRES_MODE_WRAP_DEBUG
...
void
DrcPresModeWrapRustAac::setParam(const DRC_PRES_MODE_WRAP_PARAM param, const int value)
{
    switch (param) {
...
DrcPresModeWrapRustAac::update()
{
    ...
    if (mDataUpdate) {
                ...
                if ((mIsStereoDownmix == false) && (mIsMonoDownmix == false)) {
                    ...
                    if (mDesTarget < progRefLevel) { // if target level > PRL
                        if (mEncoderTarget < mDesTarget) { // if mEncoderTarget > target level
                            ...
                            // new AttFactor will be always = calcFactor, as it was set to 0.
        ...
        if (newHeavy == 1) {
            newBoostFactor = 127; // not really needed
            newAttFactor = 127;
        }
        ...
        if (newAttFactor != mLastAttFactor) {
            ...
            if (newAttFactor != mDesAttFactor)
                ALOGV("DRC presentation mode wrapper: forced attenuation factor to %d "

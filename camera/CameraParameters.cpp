...
void CameraParameters::set(const char *key, const char *value)
{
    ...
    // Qualcomm camera HALs default to delivering an extra zero-exposure frame on HDR.
    // Android SDK only wants one frame, so disable this unless the app explicitly asks for it.
    if (!get("hdr-need-1x")) {
...
int CameraParameters::getBrightnessLumaTargetSet(int *brightness, int *luma) const {
    const char *p = get("brightness-luma-target-set");
    *brightness = *luma = -1;
    if (p == 0) return -1;
